...
def generate_summary_text(score_data: dict, recommendations: list) -> str:
    ...
    top_summaries = {
        (True, True): "各项指标基本在健康范围内，说明你目前的睡眠习惯是有效的。下面的建议可以帮助你巩固这些好习惯。",
        (True, False): "核心指标尚可，但部分指标有改善空间。看看下面的建议，针对性调整效果会更好。",
        (False, True): "关键指标偏离了健康范围，建议重点关注。下面的建议基于你的数据给出了具体的改善方向。",
        (False, False): "多项指标偏离健康范围，需要认真对待。建议逐条对照下面的建议，必要时咨询睡眠专科医生。",
    }
    # Bottom summary (at end, say something different)
    bottom_summaries = {
        (True, True): "当前睡眠状态良好，建议持续保持规律的作息时间。如果出现连续多天睡眠变差的情况，可以重新测量一次，观察趋势变化。",
        (True, False): "睡眠质量有提升空间，建议从下面最容易做到的一两条建议开始，坚持一两周后重新测量对比效果。",
        (False, True): "改善睡眠需要时间，建议按优先级逐步调整。坚持记录睡眠数据，两周后对比变化会更清楚是否有效。",
        (False, False): "如果按照建议调整后睡眠仍无明显改善，建议带着你的睡眠记录去咨询睡眠专科医生，数据会帮助医生更准确地判断。",
    }
